app.controller('photo-resource', function ($scope, $http) {
    ...
    $scope.getAllFile = function () {
        ...
        $http.get(`http://localhost:8080/PRO2111_FALL2022/TestSendMail/get-all-file/product`)
            .then(resp => {
    ...
    $scope.imageLstChanged = async function (multipleFiles) {
        ...
        await $http.post(`http://localhost:8080/PRO2111_FALL2022/TestSendMail/upload/multiple/files`, fd, {
    ...
    $scope.apiRemove = function (folder, fileName) {
        ...
        $http.delete(`http://localhost:8080/PRO2111_FALL2022/TestSendMail/delete-file/product/${fileName}`)
            .then(resp => {
                $scope.isLoading = false;
                Swal.fire(
                    'Deleted!',
