app.controller('order-ctrl', function ($scope, $http, $window, $rootScope, $timeout) {
    ...
    const API_PRODUCT_VARIANT = 'http://localhost:8080/PRO2111_FALL2022/admin/rest/product-variants';
    const API_VARIANT_VALUE = 'http://localhost:8080/PRO2111_FALL2022/admin/rest/variant-values';
    const API_BILL = 'http://localhost:8080/PRO2111_FALL2022/admin/rest/bills';
    const API_BILL_DETAIL = 'http://localhost:8080/PRO2111_FALL2022/admin/rest/bill-details';
    const API_USER = 'http://localhost:8080/PRO2111_FALL2022/admin/rest/users';
    const API_SALE = 'http://localhost:8080/PRO2111_FALL2022/admin/rest/sales';
    const API_DOWNLOAD_QR_CODE = 'http://localhost:8080/PRO2111_FALL2022/download-QRCode/bill';
    const API_PDF_BILL = 'http://localhost:8080/PRO2111_FALL2022/pdf/print';
    const API_SETTING = 'http://localhost:8080/PRO2111_FALL2022/admin/rest/settings/get-district-id-store';
    const API_VNPAY = 'http://localhost:8080/PRO2111_FALL2022/api/vnpay';
    ...
    $scope.create = async function () {
        ...
        console.log($scope[tabBill].note);
        if ($scope[tabBill].note == undefined || $scope[tabBill].note == null || $scope[tabBill].note == '') {
            $scope.isLoading = true;
            await $http.get(`${API_PRODUCT_VARIANT}/get-product-variant-of-sale`, {
                headers: { 'Authorization': 'Bearer ' + $scope.authToken }
            })
                    ...
                    .then(resp => {
                        ...
                        $scope[variable].forEach(pv => {
                                if (item.variantId == pv.productVariants.variantId) {
                                    ...
                                    if (item.sales != 0) {
                                        if (pv.price != (item.price - item.sales) && pvExist != 1) {
                                        ...
                                        if (pv.price != item.price && pvExist != 1) {
                                            $scope.messageNote += "\n Sản phẩm đã thay đổi giá theo giá gốc, mời bạn nhập ghi chú";
                                            $scope.check = 1;
                                            pvExist = 1;
                                            return;
    ...
    async function getDivisionAfterVnPay(addressInput) {
        tabAddress = 'address' + count;
        var address = {
            division: {},
            district: {},
